...
mobileinp.addEventListener('blur', (event) => {
    ...
    if (mobile.match(mobilerex)==null && mobile.length!=0 &&mobile.length!=10){
...
registerbtn.addEventListener('click',(event)=>{
    if (html!='' || emailinp.value.length==0 || mobileinp.value.length==0|| confirmpasswordinp.value.length==0 ||passwordinp.value.length==0 ){
